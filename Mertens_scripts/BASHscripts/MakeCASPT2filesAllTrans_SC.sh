...
do
    egrep -n "Start Module: casp|Stop Module:  casp" $OUT | cut -f1 -d: > SectionLines.txt
    #i=0
    #while [ $i -lt ${#LineNumbers[@]} ]
    #do
    #echo ${#CASPT2Lines[@]}
    egrep -n 'ATVX     1|ATVX     2|BVATP    1|BVATP    2|BVATM    1|BVATM    2|VJTU     1|VJTU     2|VJTIP    1|VJTIP    2|VJTIM    1|VJTIM    2|AIVX     1|AIVX     2|VJAIP    1|VJAIP    2|BJATP    1|BJATP    2|BJATM    1|BJATM    2|BJAIP    1|BJAIP    2|BJAIM    1|BJAIM    2' $OUT | sed $'s/:/\t/g' | sed -e 's/\<BVATM\>//g' | sed -e 's/\<BVATP\>//g' | sed -e 's/\<VJTIP\>//g' |sed -e 's/\<VJTIM\>//g' |sed -e 's/\<AIVX\>//g' |sed -e 's/\<VJAIP    1\>//g' |sed -e 's/\<VJAIP    2\>//g' |sed -e 's/\<VJAIM    1\>//g' |sed -e 's/\<VJAIM    2\>//g' |sed -e 's/\<BJATP    1\>//g' |sed -e 's/\<BJATP    2\>//g' |sed -e 's/\<BJATM    1\>//g' |sed -e 's/\<BJATM    2\>//g' |sed -e 's/\<BJAIP    1\>//g' | sed -e 's/\<BJAIP    2\>//g' |sed -e 's/\<BJAIM    1\>//g' |sed -e 's/\<BJAIM    2\>//g' | awk -v OFS='\t' '{print $1, $6, $9}' > Denominators.txt
    ...
    grep '++ CASPT2' $OUT | awk '{print $4}' > RootNr.txt 
    egrep -n '\+\+ CASPT2|CASPT2 PROPERTY'  $OUT | cut -f1 -d: > RootLines.txt
    #CaseLines=($(egrep -n 'CASE' $OUT | cut -f1 -d:))
    grep 'State symmetry' $OUT | awk '{print $3}' > Symm.txt
    grep 'Spin quantum' $OUT | awk '{print $4}' > Spin.txt
    grep 'Number of root(s) av' $OUT | awk '{print $5}' > NumberOfRoots.txt
    echo ${OUT#*_} > OutputFiles.txt
    #done
    GetIntruderLines.py
done
